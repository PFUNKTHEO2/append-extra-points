"""
Factor 28: NHL Scouting Report (F28 NHLSR)
==========================================
Creates the F28 NHL Scouting Report factor from Central Scouting Mid-Term Rankings.
...
Lists:
1. North American Skaters (224 players + 1 LV)
2. North American Goalies (37 players)
3. International Skaters (128 players)
4. International Goalies (20 players)
"""
...
def calculate_points(rank, total_players, max_points=1000, min_points=500):
    ...
    Rank 1 gets max_points, last rank gets min_points.
    """
    if total_players == 1:
        return max_points
    # Linear interpolation: points = max - (rank - 1) * (max - min) / (total - 1)
...
# North American Skaters (224 ranked + 1 Limited Viewing)
na_skaters = [
...
def create_f28_dataframe():
    ...
    # Process North American Skaters
    total_na_skaters = len(na_skaters)
    for rank, name, team, league, birthdate, pos in na_skaters:
        points = calculate_points(rank, total_na_skaters)
        all_data.append({
            'nhl_scouting_rank': rank,
            'list_type': 'NA_SKATERS',
            'player_name': name,
            'team': team,
            'league': league,
            'birthdate': birthdate,
            ...
            'total_in_list': total_na_skaters
        })

    # Process North American Goalies
    total_na_goalies = len(na_goalies)
    for rank, name, team, league, birthdate, pos in na_goalies:
        points = calculate_points(rank, total_na_goalies)
        all_data.append({
            'nhl_scouting_rank': rank,
            'list_type': 'NA_GOALIES',
            'player_name': name,
            'team': team,
            'league': league,
            'birthdate': birthdate,
            'position': pos,
            'factor_28_nhl_scouting_points': points,
            'total_in_list': total_na_goalies
        })

    # Process International Skaters
    total_intl_skaters = len(intl_skaters)
    for rank, name, team, league, birthdate, pos in intl_skaters:
        points = calculate_points(rank, total_intl_skaters)
        all_data.append({
            'nhl_scouting_rank': rank,
            'list_type': 'INTL_SKATERS',
            'player_name': name,
            'team': team,
            'league': league,
            'birthdate': birthdate,
            'position': pos,
            'factor_28_nhl_scouting_points': points,
            'total_in_list': total_intl_skaters
        })

    # Process International Goalies
    total_intl_goalies = len(intl_goalies)
    for rank, name, team, league, birthdate, pos in intl_goalies:
        points = calculate_points(rank, total_intl_goalies)
        all_data.append({
            'nhl_scouting_rank': rank,
            'list_type': 'INTL_GOALIES',
            'player_name': name,
            'team': team,
            'league': league,
            'birthdate': birthdate,
            'position': pos,
            'factor_28_nhl_scouting_points': points,
            'total_in_list': total_intl_goalies
        })
...
def match_players_to_bigquery(df):
    """Match players to BigQuery player_stats using name and birth year."""
    ...
    SELECT
        ...
        latestStats_teamName as bq_team
    ...
    def extract_birth_year(birthdate):
        try:
    ...
    for idx, row in df.iterrows():
        ...
        matches = bq_players[
            (bq_players['normalized_name'] == row['normalized_name']) &
            (bq_players['birth_year'] == row['birth_year'])
        ]

        if len(matches) == 1:
            matched.append({
                **row.to_dict(),
                'player_id': int(matches.iloc[0]['player_id']),
                'match_type': 'exact'
            })
        elif len(matches) > 1:
            # Multiple matches - pick first one (could enhance logic here)
            matched.append({
                **row.to_dict(),
                'player_id': int(matches.iloc[0]['player_id']),
            ...
            # Try fuzzy matching on name only
            name_matches = bq_players[
                bq_players['normalized_name'] == row['normalized_name']
            ]
            if len(name_matches) == 1:
                matched.append({
                    **row.to_dict(),
                    'player_id': int(name_matches.iloc[0]['player_id']),
...
def upload_to_bigquery(df, table_name):
    ...
    # Keep only necessary columns for the final table
    upload_df = df[[
        'player_id',
        'nhl_scouting_rank',
        'list_type',
...
def main():
    print("="*60)
    print("Factor 28: NHL Scouting Report (F28 NHLSR)")
    print("="*60)

    # Step 1: Create DataFrame from PDF data
    ...
    print(f"  Total players: {len(df)}")
    print(f"\n  By List Type:")
    for list_type, count in df['list_type'].value_counts().items():
        print(f"    {list_type}: {count}")

    print(f"\n  Points Range by List:")
    for list_type in df['list_type'].unique():
        subset = df[df['list_type'] == list_type]
        print(f"    {list_type}: {subset['factor_28_nhl_scouting_points'].min():.1f} - {subset['factor_28_nhl_scouting_points'].max():.1f}")

    # Step 2: Save raw CSV for reference
    raw_csv_path = "F28_nhl_scouting_raw.csv"
    df.to_csv(raw_csv_path, index=False)
    print(f"\n2. Saved raw data to {raw_csv_path}")

    # Step 3: Match to BigQuery player IDs
    print("\n3. Matching players to BigQuery player_stats...")
    matched_df, unmatched_df = match_players_to_bigquery(df)

    # Save matched and unmatched for review
    if len(matched_df) > 0:
        matched_df.to_csv("F28_nhl_scouting_matched.csv", index=False)
        print(f"   Saved matched players to F28_nhl_scouting_matched.csv")

    if len(unmatched_df) > 0:
        unmatched_df.to_csv("F28_nhl_scouting_unmatched.csv", index=False)
        print(f"   Saved unmatched players to F28_nhl_scouting_unmatched.csv")
    ...
    if len(matched_df) > 0:
        ...
        # Show top players
        print("\nTop 10 Players by F28 Points:")
        top10 = matched_df.nlargest(10, 'factor_28_nhl_scouting_points')
        for _, row in top10.iterrows():
            print(f"  {row['player_name']}: {row['factor_28_nhl_scouting_points']:.1f} pts ({row['list_type']})")

    print("\n" + "="*60)
    print("DONE! Next steps:")
    print("1. Run create_PT_F28_NHLSR_table.sql to create the final table")
    print("2. Update rebuild_cumulative_with_fixes.sql to include F28")
    print("3. Rebuild player_cumulative_points table")
    print("="*60)
