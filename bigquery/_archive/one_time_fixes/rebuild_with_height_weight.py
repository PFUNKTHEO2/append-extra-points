"""
Rebuild player_cumulative_points with height_cm, weight_kg, and NHL scouting columns
"""
...
SELECT
  ...
  (
    COALESCE(f01.f01_views, 0.0) + COALESCE(f02.f02_height, 0.0) +
    COALESCE(f03.f03_current_goals_f, 0.0) + COALESCE(f04.f04_current_goals_d, 0.0) +
    COALESCE(f05.f05_current_assists, 0.0) + COALESCE(f06.f06_current_gaa, 0.0) +
    COALESCE(f07.f07_current_svp, 0.0) + COALESCE(f08.f08_last_goals_f, 0.0) +
    COALESCE(f09.f09_last_goals_d, 0.0) + COALESCE(f10.f10_last_assists, 0.0) +
    COALESCE(f11.f11_last_gaa, 0.0) + COALESCE(f12.f12_last_svp, 0.0)
  ) AS performance_total,

  (
    COALESCE(f13.f13_league_points, 0) + COALESCE(f14.f14_team_points, 0) +
    COALESCE(f15.f15_international_points, 0.0) + COALESCE(f16.f16_commitment_points, 0) +
    COALESCE(f17.f17_draft_points, 0) + COALESCE(f18.f18_weekly_points_delta, 0.0) +
    COALESCE(f19.f19_weekly_assists_delta, 0.0) + COALESCE(f20.f20_playing_up_points, 0) +
    COALESCE(f21.f21_tournament_points, 0) + COALESCE(f22.f22_manual_points, 0) +
    COALESCE(f23.f23_prodigylikes_points, 0) + COALESCE(f24.f24_card_sales_points, 0) +
    COALESCE(f25.f25_weekly_views, 0.0) + COALESCE(f26.f26_weight_points, 0.0) +
    COALESCE(f27.f27_bmi_points, 0.0) + COALESCE(f28.f28_nhl_scouting_points, 0.0)
  ...
  'v3.0-with-height-weight' AS algorithm_version
...
print('Rebuilding cumulative table with height_cm, weight_kg, and NHL scouting columns...')
client.query(rebuild_sql).result()
print('Done!')

# Verify
count = list(client.query('SELECT COUNT(*) as cnt FROM algorithm_core.player_cumulative_points').result())[0].cnt
print(f'Total players: {count:,}')

# Check new columns
sample = list(client.query('''
SELECT player_name, height_cm, weight_kg, nhl_scouting_rank, nhl_scouting_list
FROM algorithm_core.player_cumulative_points
WHERE nhl_scouting_rank IS NOT NULL
LIMIT 3
''').result())
print('\nSample NHL scouting players:')
for s in sample:
    print(f'  {s.player_name}: {s.height_cm}cm, {s.weight_kg}kg, NHL rank {s.nhl_scouting_rank} ({s.nhl_scouting_list})')
