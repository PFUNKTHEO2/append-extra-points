#!/usr/bin/env python3
"""
Final Draft Data Upsert Script
Processes Canadian and USHL draft data and uploads to BigQuery
"""
...
class DraftDataProcessor:
    def __init__(self, project_id: str = "hockey-data-analysis"):
...
def main():
    ...
    try:
        ...
        if not os.path.exists(canadian_file):
            print(f"ERROR: Canadian file not found: {canadian_file}")
            return
        if not os.path.exists(ushl_file):
            print(f"ERROR: USHL file not found: {ushl_file}")
            return
